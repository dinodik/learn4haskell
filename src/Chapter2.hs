...
subList start end list
    ...
    | start == end = take 1 (drop start list)
    | otherwise = take 1 (drop start list) ++ subList (start + 1) end list
...
firstHalf :: [a] -> [a]
firstHalf l = subList 0 half l
    where half = length l `div` 2 - 1
...
Closest exponent in 2^x to get to n?
